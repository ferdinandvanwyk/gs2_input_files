...
#                                                                    #
# Created 2015-12-05 16:28:25 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.004802387151100563    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.5900074616441469    # Effective ionic charge.
...
@shat = 3.9993326663970947    # 
@rhoc = 0.7966437935829163    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 2.314927234164804    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.4612954770153308    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.4910101890563965    # akappri = dkappa/drho
@tri = 0.21148361345157765    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.5176069736480713    # tripri =  dtri/drho
@shift = -0.3070124956221228    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4891069751742136    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 3.9993326663970947    # 
@beta_prime_input = -0.17290526628494263    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 5.608509063720703    # -1/T (dT/drho)
@fprim_1 = 2.613342046737671    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.012936392596194462    # collisionality parameter
...
@dens_2 = 1.141480018969939    # Density	
@temp_2 = 1.0915337400203502    # Temperature
@tprim_2 = 5.820106506347656    # -1/T (dT/drho)
@fprim_2 = 2.3871495723724365    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.6880340057258691    # collisionality parameter
