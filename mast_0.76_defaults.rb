...
#                                                                    #
# Created 2015-12-05 16:28:58 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.003117306091945652    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.6664800055065037    # Effective ionic charge.
...
@shat = 4.919582843780518    # 
@rhoc = 0.8539687991142273    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 3.057943642736144    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.495296246382856    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.8162449598312378    # akappri = dkappa/drho
@tri = 0.24371119897283172    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.7190771698951721    # tripri =  dtri/drho
@shift = -0.3492713770679674    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.470273778716218    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 4.919582843780518    # 
@beta_prime_input = -0.10191579163074493    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 5.0716047286987305    # -1/T (dT/drho)
@fprim_1 = 3.178347587585449    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.019251204700348367    # collisionality parameter
...
@dens_2 = 1.1580856556764711    # Density	
@temp_2 = 1.0461302777697512    # Temperature
@tprim_2 = 6.819929599761963    # -1/T (dT/drho)
@fprim_2 = 2.8504478931427    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 1.0912687464562416    # collisionality parameter
