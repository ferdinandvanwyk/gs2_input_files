...
#                                                                    #
# Created 2015-12-05 16:29:08 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.002461598256110481    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.7200529817477235    # Effective ionic charge.
...
@shat = 5.35648775100708    # 
@rhoc = 0.8799737691879272    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 3.677886767808074    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.5244715355451108    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 1.0853278636932373    # akappri = dkappa/drho
@tri = 0.26795229555068173    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.8380530476570129    # tripri =  dtri/drho
@shift = -0.36429695453080996    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4609493873207517    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 5.35648775100708    # 
@beta_prime_input = -0.06440988183021545    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 3.0189337730407715    # -1/T (dT/drho)
@fprim_1 = 3.5484445095062256    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.02229176465107605    # collisionality parameter
...
@dens_2 = 1.17119169033499    # Density	
@temp_2 = 0.9569706503810083    # Temperature
@tprim_2 = 7.509772777557373    # -1/T (dT/drho)
@fprim_2 = 3.1469926834106445    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 1.444271977160593    # collisionality parameter
