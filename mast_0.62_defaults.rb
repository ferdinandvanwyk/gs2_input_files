...
#                                                                    #
# Created 2015-12-05 16:27:00 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.008433129521202074    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.4909910245050868    # Effective ionic charge.
...
@shat = 2.6656689643859863    # 
@rhoc = 0.7149277329444885    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 1.6385928073566143    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.4328002841154561    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.24657221138477325    # akappri = dkappa/drho
@tri = 0.1780794028775145    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.34099265933036804    # tripri =  dtri/drho
@shift = -0.2581843992577456    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.5119517932421789    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 2.6656689643859863    # 
@beta_prime_input = -0.269357293844223    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 3.9060025215148926    # -1/T (dT/drho)
@fprim_1 = 2.0846879482269287    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.0074143186408655365    # collisionality parameter
...
@dens_2 = 1.125914538238934    # Density	
@temp_2 = 1.127498613611374    # Temperature
@tprim_2 = 4.77232551574707    # -1/T (dT/drho)
@fprim_2 = 1.9642947912216187    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.3756208446804916    # collisionality parameter
