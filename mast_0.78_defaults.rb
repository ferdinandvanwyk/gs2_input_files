...
#                                                                    #
# Created 2015-12-05 16:29:03 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.00275211315103774    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.6930833722356473    # Effective ionic charge.
...
@shat = 5.152667999267578    # 
@rhoc = 0.8671802282333374    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 3.3544596153436372    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.5092575748480377    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.911900520324707    # akappri = dkappa/drho
@tri = 0.25561225317753    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.7691414952278137    # tripri =  dtri/drho
@shift = -0.3585315634542409    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4655971900772564    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 5.152667999267578    # 
@beta_prime_input = -0.08870930224657059    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 4.504329204559326    # -1/T (dT/drho)
@fprim_1 = 3.3455874919891357    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.02102815181934876    # collisionality parameter
...
@dens_2 = 1.1644551680160518    # Density	
@temp_2 = 1.0083028140324755    # Temperature
@tprim_2 = 7.07047700881958    # -1/T (dT/drho)
@fprim_2 = 2.9885518550872803    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 1.259699909844483    # collisionality parameter
