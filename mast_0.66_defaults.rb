...
#                                                                    #
# Created 2015-12-05 16:28:03 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.006455921376992446    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.5403879607471267    # Effective ionic charge.
...
@shat = 3.4897563457489014    # 
@rhoc = 0.7652621865272522    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 1.93677080077539    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.444688969845241    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.360721230506897    # akappri = dkappa/drho
@tri = 0.1933350040985525    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.4278683066368103    # tripri =  dtri/drho
@shift = -0.2852171743541018    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4983967972222543    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 3.4897563457489014    # 
@beta_prime_input = -0.22063636779785156    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 4.9965033531188965    # -1/T (dT/drho)
@fprim_1 = 2.3498196601867676    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.00959058664970581    # collisionality parameter
...
@dens_2 = 1.1325634582581967    # Density	
@temp_2 = 1.1003211583245613    # Temperature
@tprim_2 = 5.332026958465576    # -1/T (dT/drho)
@fprim_2 = 2.172006368637085    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.5039859627752781    # collisionality parameter
