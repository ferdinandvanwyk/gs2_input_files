...
#                                                                    #
# Created 2015-12-05 16:28:33 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.004135872202715343    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.6151041730191749    # Effective ionic charge.
...
@shat = 4.24199914932251    # 
@rhoc = 0.8116511702537537    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 2.5409964008211174    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.4713792982937581    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.6219577193260193    # akappri = dkappa/drho
@tri = 0.22157572770268857    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.5986363291740417    # tripri =  dtri/drho
@shift = -0.3190650401698559    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4844143505642031    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 4.24199914932251    # 
@beta_prime_input = -0.12880904972553253    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 5.463542938232422    # -1/T (dT/drho)
@fprim_1 = 2.7892796993255615    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.014982787991846356    # collisionality parameter
...
@dens_2 = 1.146593899805394    # Density	
@temp_2 = 1.0842966245625596    # Temperature
@tprim_2 = 6.090062141418457    # -1/T (dT/drho)
@fprim_2 = 2.522188663482666    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.8048648082400126    # collisionality parameter
