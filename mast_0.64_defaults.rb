...
#                                                                    #
# Created 2015-12-05 16:27:07 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.007416942376370318    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.5156791559401364    # Effective ionic charge.
...
@shat = 2.9590327739715576    # 
@rhoc = 0.732117235660553    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 1.778067339151665    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.4381756672233057    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.2972007989883423    # akappri = dkappa/drho
@tri = 0.18534568650710498    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.37789151072502136    # tripri =  dtri/drho
@shift = -0.26788921239988167    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.507497736562119    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 2.9590327739715576    # 
@beta_prime_input = -0.2407039850950241    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 4.426634311676025    # -1/T (dT/drho)
@fprim_1 = 2.1747448444366455    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.008356387931397459    # collisionality parameter
...
@dens_2 = 1.1289154061514721    # Density	
@temp_2 = 1.1095451636146518    # Temperature
@tprim_2 = 4.979237079620361    # -1/T (dT/drho)
@fprim_2 = 2.027433156967163    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.4336642077478826    # collisionality parameter
