...
#                                                                    #
# Created 2015-12-05 16:28:39 +0000                                           #
#                                                                    #
######################################################################
...
@beta = 0.0035752729049079563    # Ratio of particle to magnetic pressure (reference Beta, not total beta):  beta=n_0 T_0 /( B^2 / (8 pi))
@zeff = 1.6406350490365977    # Effective ionic charge.
...
@shat = 4.443373203277588    # 
@rhoc = 0.8262051939964294    # Flux surface label. Usually rho = diameter/diameter of LCFS
@qinp = 2.7876954138835184    # Sets value of the safety factor when using local toroidal equilibrium model.
@akappa = 1.4826712910879172    # Sets local elongation when local toroidal equilibrium is specified.
@akappri = 0.7030388116836548    # akappri = dkappa/drho
@tri = 0.23233473420342807    # tri = arcsin[(R(max(Z))-R_major)/r_mid]
@tripri = 0.6478594541549683    # tripri =  dtri/drho
@shift = -0.32902675423121014    # Sets Shafranov shift. See online help for definition.
@rmaj = 1.4797030611354307    # Major radius/a (Position of magnetic axis)
...
@local_eq = ".true."    # 
@bishop = 4    # 
@s_hat_input = 4.443373203277588    # 
@beta_prime_input = -0.11274076253175735    # The gradient of the pressure.
@delrho = 0.001    # 
@equal_arc = ".false."    # 
...
@tprim_1 = 5.242146968841553    # -1/T (dT/drho)
@fprim_1 = 2.9338557720184326    # -1/n (dn/drho)
@uprim_1 = 0.0    # ?
@vnewk_1 = 0.017148695092043968    # collisionality parameter
...
@dens_2 = 1.1521501186177763    # Density	
@temp_2 = 1.0704400180348093    # Temperature
@tprim_2 = 6.307665824890137    # -1/T (dT/drho)
@fprim_2 = 2.642397880554199    # -1/n (dn/drho)
@uprim_2 = 0.0    # ?
@vnewk_2 = 0.9391609723601286    # collisionality parameter
